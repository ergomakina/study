...
void* client_thread(void* arg)
{
    ...
    socklen_t addrlen;
    char buf[1024], hostname[128], portname[128];
    int clientfd = (int)(intptr_t)arg;
    int len, i, wlen, er;

    addrlen = sizeof(addr);
    er = getpeername(clientfd, (struct sockaddr*)&addr, &addrlen);
    if (er < 0) goto error;

    er = getnameinfo((struct sockaddr*)&addr, addrlen, hostname, sizeof(hostname),
