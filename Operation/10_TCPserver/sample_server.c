...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        int rc = getnameinfo((struct sockaddr *)&addr,
                             addrlen, hoststr, sizeof(hoststr), portstr, sizeof(portstr),
                             NI_NUMERICHOST | NI_NUMERICSERV);
        send(clientfd, "Your address is ", sizeof("Your address is "), 0);
        send(clientfd, hoststr, sizeof(hoststr), 0);
        send(clientfd, " at port ", sizeof(" at port "), 0);
        send(clientfd, portstr, sizeof(portstr), 0);
        send(clientfd, "\n", sizeof("\n"), 0);
